...
contract BridgeRelay {
    ...
    function bridgeTransfer(IERC20 token) external payable {
        // @audit : how the token are suposed to be transfered? address(this)

        // revert if MATIC is attempted
        if (token == MATIC) revert MATICUnbridgeable();
        // unwrap WETH
        if (token == WETH) {
            // WETH => deposit => ether return WETH , withdraw => WETH return ether
        ...
        // transfer ETHER
        // @audit : will also transfer donated ETH.
    ...
    function erc20Rescue(address destination) external {
        ...
        //transfer MATIC
        // transfer => bool return : return false , revert
