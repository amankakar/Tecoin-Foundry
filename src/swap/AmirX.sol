...
import {SwapTypes} from "./interfaces/SwapTypes.sol";
 ...
 */
contract AmirX is SwapTypes , StablecoinHandler {
    ...
    function stablecoinSwap(
        ...
        //eXYZ ot eXYZ
        //@audit :  both token are valid stablecoin
        ...
        //stablecoin swap
        // @audit : orign is valid but traget is not valid
        if (isXYZ(ss.origin) && !isXYZ(ss.target))
            convertFromEXYZ(wallet, safe, ss);
            console.log("heree");

        //defi swap
        // @audit : try to execute both if in a single transaction and see what could happen?
    ...
    function _feeDispersal(address safe, DefiSwap memory defi) internal {
        ...
        if (defi.referrer != address(0) && defi.referralFee != 0) {
            console.log("njnj" , TELCOIN.balanceOf(safe));
        ...
        // assert here the core invariant for fee
        // retain remainder
        console.log("done");
